...
function handleEnterPlatform() {
    ...
    // إظهار الفوتر (إزالة الإخفاء للأسفل + إظهار الشفافية)
    setTimeout(() => {
...
function switchView(viewId) {
    ...
    // 1. إدارة الفوتر (يظهر في الرئيسية والاختيار فقط)
    // نستخدم كلاسات Tailwind للتحريك
    ...
    gradeData.subjects.forEach(subject => {
        ...
        card.onclick = () => {
            if (isLocked) {
                ...
                // إخفاء العداد إذا كان القسم مغلقاً
...
function applyLockVisuals(element, isLocked) {
    ...
    if (isLocked) {
        element.classList.add('opacity-80', 'bg-gray-50');
        // نحتفظ بتأثيرات الهوفر البسيطة لكن نزيل الحدود الملونة الخاصة
        element.classList.remove('hover:border-red-100', 'hover:border-green-100');
        ...
        element.classList.remove('opacity-80', 'bg-gray-50');
...
function renderLessons(lessons) {
    ...
    // إخفاء زر البدء
    const quizBtn = document.getElementById('startQuizContainer');
    quizBtn.classList.remove('is-visible');
    quizBtn.classList.add('is-hidden');
