...
function handleEnterPlatform() {
    ...
    // إظهار الفوتر بعد ثانية (إزالة الشفافية والإزاحة)
    setTimeout(() => {
        footer.classList.remove('translate-y-4', 'opacity-0');
    }, 1000);
}
...
function switchView(viewId) {
    ...
    // التحكم في الفوتر: إذا لم تكن الصفحة الرئيسية أو صفحة الاختيار -> أخفِه
    if (viewId === 'homeView' || viewId === 'modeView') {
        footer.classList.remove('translate-y-4', 'opacity-0');
    } else {
        footer.classList.add('translate-y-4', 'opacity-0');
    }
